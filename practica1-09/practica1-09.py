...
def drawing_output(frame, coordinates_left_eye, coordinates_right_eye, blink_counter):
    ...
    cv2.rectangle(output, (0, 0), (265, 50), (255, 0, 0), 2)
...
def eye_aspect_ratio(coordinates):
    ...
    d_C = np.linalg.norm(np.array(coordinates[0]) - np.array(coordinates[3]))  # corregido
...
def plotting_ear(pts_ear, line1):
    ...
    if line1 == []:
        ...
        plt.ion()
        figure, ax = plt.subplots()  # corregido
...
EAR_THRESH = 0.20    #por medio de este se hacen algunos ajustes para el resultado final. ya que da falsos positivos
NUM_FRAMES = 2       #velocidad
aux_counter = 0
blink_counter = 0
line1 = []
...
with mp_face_mesh.FaceMesh(static_image_mode=False, max_num_faces=1) as face_mesh:
    while True:
        ...
        if results.multi_face_landmarks is not None:  # corregido
            for face_landmarks in results.multi_face_landmarks:
                for index in index_left_eye:
                    x = int(face_landmarks.landmark[index].x * width)  # corregido
                    y = int(face_landmarks.landmark[index].y * height)  # corregido
                    coordinates_left_eye.append([x, y])
                    cv2.circle(frame, (x, y), 2, (0, 255, 255), 1)
                    cv2.circle(frame, (x, y), 1, (128, 0, 250), 1)  # corregido
                ...
                for index in index_right_eye:
                    ...
                    cv2.circle(frame, (x, y), 1, (0, 255, 255), 1)  # corregido
...
cap.release()
cv2.destroyAllWindows()  # corregido 